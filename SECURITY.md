...
- Default gateway config: `config/ultron_connect.json`
...
- Optional mDNS discovery is provided through `ultron_connect.gateway.discovery.GatewayDiscovery` and Zeroconf.
- Discovery advertises service `_ULTRON._tcp.local.` only when the Zeroconf library is installed and `advertise` is enabled.
...
- Pairing uses a temporary pairing offer created by `ultron_connect.gateway.pairing.PairingManager`.
...
- Device records are stored in `config/ultron_connect/devices.json`.
...
- Disable `advertise` in `config/ultron_connect.json` unless discovery is needed.
