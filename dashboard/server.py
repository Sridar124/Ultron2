...
def _ensure_ssl_certs() -> bool:
    ...
    key_path = certs / "ultron.key"
    cert_path = certs / "ultron.crt"
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Ultron AI Local Remote"),
        ])
        alt_names = [
...
class DashboardServer:
    ...
    def _build_app(self) -> "FastAPI":
        ...
        _MOBILE_DISABLED_HTML = """<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Ultron — Access Disabled</title>
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  body{background:#000;color:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
       display:flex;align-items:center;justify-content:center;height:100vh}
  .card{text-align:center;padding:48px 40px;border:1px solid rgba(220,38,38,0.3);
        border-radius:20px;background:rgba(220,38,38,0.05);max-width:360px}
  .icon{font-size:48px;margin-bottom:20px}
  h1{font-size:20px;font-weight:700;color:#ef4444;margin-bottom:10px;letter-spacing:2px}
  p{font-size:13px;color:rgba(255,255,255,0.55);line-height:1.6}
</style></head>
<body><div class="card">
  <div class="icon">🔒</div>
  <h1>MOBILE ACCESS DISABLED</h1>
  <p>Remote and mobile access has been disabled on this Ultron installation.<br>
     Please use the desktop application directly.</p>
</div></body></html>"""

        @app.get("/login", response_class=HTMLResponse)
        async def login_page():
            return HTMLResponse(_MOBILE_DISABLED_HTML, status_code=403)

        @app.get("/", response_class=HTMLResponse)
        async def index():
            # Local desktop use only — serve the full app shell
            html = (self._app_html
        ...
        async def login(req: Request):
            return JSONResponse(
                {"ok": False, "error": "Mobile access is disabled on this installation."},
                status_code=403,
            )
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = get_user_data_dir() / "config" / "certs" / "ultron.key"
        ssl_cert = get_user_data_dir() / "config" / "certs" / "ultron.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Ultron UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
