...
from .gateway.server import UltronGateway, UltronGatewayConfig


@dataclass(slots=True)
class UltronConnectService:
    """Small runtime wrapper around the gateway server."""

    base_dir: Path
    config: UltronGatewayConfig | None = None
    gateway: UltronGateway = field(init=False)
    _thread: threading.Thread | None = field(init=False, default=None)
    _started: bool = field(init=False, default=False)

    def __post_init__(self) -> None:
        self.base_dir = Path(self.base_dir)
        self.gateway = UltronGateway(self.base_dir, self.config)
    ...
    def start_background(self) -> None:
        ...
        self._thread = threading.Thread(target=_runner, name="UltronConnectGateway", daemon=True)
        self._thread.start()
        self._started = True
    ...
    async def disconnect_device(self, target: str, *, reason: str = "Disconnected by Ultron") -> dict[str, Any]:
...
_SERVICE: UltronConnectService | None = None


def get_service(base_dir: str | Path) -> UltronConnectService:
    global _SERVICE
    if _SERVICE is None:
        _SERVICE = UltronConnectService(Path(base_dir))
    return _SERVICE
