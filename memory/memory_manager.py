...
def should_extract_memory(user_text: str, ultron_text: str, api_key: str = "") -> bool:
    try:
        from or_client import client

        combined = f"User: {user_text[:300]}\nUltron AI: {ultron_text[:1000]}"
...
def extract_memory(user_text: str, ultron_text: str, api_key: str = "") -> dict:
    try:
        from or_client import client

        combined = f"User: {user_text[:600]}\nUltron AI: {ultron_text[:300]}"
        ...
        raw = client.chat(
            ...
            f"                  (e.g. ultron_ai: 'Building a Ultron AI assistant')\n"
            ...
            f"- Extract from BOTH user and Ultron AI turns.\n"
            ...
            f' "projects":{{"ultron_ai":{{"value":"Ultron AI assistant"}}}},\n'
...
def all_entries_for_ui() -> list[dict]:
    """Flat list for the memory panel: what Ultron knows, and when it learned it.
...
def auto_learn_interaction(user_text: str, ultron_text: str = "") -> dict:
    """
    ...
    if m_name:
        ...
        if name_val.lower() not in ("ultron", "sir", "user", "admin", "echo", "assistant", "here"):
