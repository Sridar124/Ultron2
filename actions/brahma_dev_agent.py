...
logger = logging.getLogger("ultron_dev_agent")
...
# ==============================================================================
# NATIVE CLAUDE-CODE TOOLS (REBRANDED FOR ULTRON DEV)
# ==============================================================================
...
# ==============================================================================
# CLAUDE-CODE LEAKED SYSTEM PROMPT (REBRANDED FOR ULTRON DEV)
# ==============================================================================

ULTRON_DEV_SYSTEM_PROMPT = """You are Ultron Dev, the expert software engineering autonomous agent built natively into Ultron AI.
...
class UltronDevAgent:
    ...
    def _call_llm(self) -> str:
        ...
        try:
            ...
            if gemini_key:
                ...
                system_instruction = ULTRON_DEV_SYSTEM_PROMPT
                contents = []
                ...
                if last_err:
                    logger.warning(f"[UltronDev] Gemini calls exhausted: {last_err}")
        except Exception as e:
            logger.warning(f"[UltronDev] Direct Gemini setup error: {e}")
    ...
    def run(self, user_instruction: str, max_turns: int = 25) -> str:
        ...
        print(f"\n[UltronDev] Starting Developer Task in {self.workspace_dir}")
        print(f"[UltronDev] Prompt: {user_instruction}\n")
        ...
        self.history = [
            {"role": "system", "content": ULTRON_DEV_SYSTEM_PROMPT + system_info},
            {"role": "user", "content": user_instruction}
        ]

        final_response = ""
        for turn in range(1, max_turns + 1):
            print(f"--- [UltronDev] Turn {turn}/{max_turns} ---")
            try:
                reply = self._call_llm()
            except Exception as e:
                err_msg = f"LLM error: {e}"
                logger.error(err_msg)
                return f"Ultron Dev encountered an error during inference: {err_msg}"
...
def run_dev_agent(parameters: dict[str, Any], speak: Optional[Callable[[str], None]] = None) -> str:
    ...
    workspace = str(params.get("workspace_path") or params.get("output_dir") or Path.home() / "Desktop" / "UltronProjects").strip()
    
    agent = UltronDevAgent(workspace_dir=workspace, speak=speak)
