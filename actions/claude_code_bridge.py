...
def run_developer_mode_request(parameters: dict[str, Any], speak=None) -> str:
    ...
    if not workspace:
        workspace = str(Path.home() / "Desktop" / "UltronProjects")
        Path(workspace).mkdir(parents=True, exist_ok=True)

    params["workspace_path"] = workspace
    params["output_dir"] = workspace

    # Run native Ultron Dev Agent powered by Claude Code architecture & tools
    try:
        return run_dev_agent(params, speak=speak)
    except Exception as exc:
        print(f"[ClaudeBridge] UltronDevAgent encountered error: {exc}, falling back to legacy dev agent")
        params.setdefault("language", params.get("language") or "python")
        params.setdefault("project_name", params.get("project_name") or "ultron_project")
