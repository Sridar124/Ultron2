...
"""
Ultron AI — Calorie & Nutrition Vision Engine.
...
def _analyze_food_multimodal(image_b64: Optional[str], query: str) -> dict:
    ...
    system_prompt = (
        "You are Ultron AI's Precision Nutrition Engine. "
...
def run(parameters: dict, player=None, speak=None, session_memory=None) -> str:
    """Main execution function for Ultron AI Calorie Counter."""
    ...
    # Display rich breakdown in Ultron UI
    panel_content = (
        f"**Dish**: {dish} ({portion})\n\n"
    ...
    if player and hasattr(player, "show_content"):
        try:
            player.show_content("🥗 ULTRON NUTRITION SCAN", panel_content)
        except Exception:
            pass
