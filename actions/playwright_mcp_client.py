...
def _get_app_data_dir() -> Path:
    """Returns local app data directory for Ultron AI."""
    if platform.system() == "Windows":
        base = os.environ.get("LOCALAPPDATA")
        if base:
            p = Path(base) / "UltronAI" / "PlaywrightProfile"
            p.mkdir(parents=True, exist_ok=True)
            return p
    home = Path.home() / ".ultron_ai" / "playwright_profile"
            ...
            try:
                init_resp = self._send_request(
                    "initialize",
                    {
                        ...
                        "capabilities": {},
                        "clientInfo": {"name": "ultron-ai", "version": "1.0.0"},
                    },
                    timeout=30,
                )
                if not init_resp:
