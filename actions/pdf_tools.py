"""
pdf_tools.py - Ultron AI PDF support
    ...
    try:
        ...
        class NumberedCanvas(canvas.Canvas):
            ...
            def draw_page_decorations(self, page_count: int):
                ...
                # Running header
                doc_title = getattr(self, "doc_title", "Ultron AI Executive Report")
                ...
                self.drawString(54, 32, "ULTRON AI - AUTONOMOUS INTELLIGENCE RESEARCH")
...
def _render_title_page(story, pdf, title: str, subtitle: str | None, styles, author: str | None = None):
    ...
    story.append(pdf["Paragraph"]("TECHNICAL RESEARCH MONOGRAPH", styles["ultron_doc_badge"]))
    story.append(pdf["Paragraph"](title, styles["ultron_doc_title"]))
    if subtitle:
        story.append(pdf["Paragraph"](subtitle, styles["ultron_doc_subtitle"]))
    story.append(pdf["Spacer"](1, 0.4 * pdf["inch"]))
    author_text = author or "User"
    story.append(
        pdf["Paragraph"](
            f"<b>Author:</b> {author_text} &nbsp;|&nbsp; <b>Division:</b> Ultron AI Autonomous Systems &nbsp;|&nbsp; <b>Date:</b> {datetime.now().strftime('%B %d, %Y')}",
            styles["ultron_doc_meta"],
        )
    )
    story.append(pdf["Spacer"](1, 0.1 * pdf["inch"]))
    story.append(
        pdf["Paragraph"](
            "<b>Classification:</b> Public Technical Monograph &nbsp;|&nbsp; <b>Engine:</b> Ultron AI Publication Core",
            styles["ultron_doc_meta"],
        )
    )
        ...
        if kind == "heading":
            ...
            if level == 1:
                if not first_heading:
                    story.append(pdf["PageBreak"]())
                first_heading = False
                story.append(pdf["Paragraph"](clean_t, styles["ultron_h1"]))
                story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
            elif level == 2:
                story.append(pdf["Paragraph"](clean_t, styles["ultron_h2"]))
                story.append(pdf["Spacer"](1, 0.05 * pdf["inch"]))
            else:
                story.append(pdf["Paragraph"](clean_t, styles["ultron_h3"]))
                story.append(pdf["Spacer"](1, 0.04 * pdf["inch"]))

        elif kind == "bullet":
            clean_t = _clean_for_reportlab(text)
            story.append(pdf["Paragraph"](f"&bull;&nbsp;&nbsp;{clean_t}", styles["ultron_bullet"]))

        elif kind == "numbered":
            clean_t = _clean_for_reportlab(text)
            story.append(pdf["Paragraph"](f"<b>{clean_t}</b>", styles["ultron_bullet"]))

        elif kind == "code":
            clean_t = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            story.append(pdf["Paragraph"](f"<pre>{clean_t}</pre>", styles["ultron_code"]))
            ...
            for r_idx, row in enumerate(rows):
                ...
                for cell in cells:
                    ...
                    if r_idx == 0:
                        row_flowables.append(pdf["Paragraph"](c_clean, styles["ultron_table_header"]))
                    else:
                        row_flowables.append(pdf["Paragraph"](c_clean, styles["ultron_table_cell"]))
            ...
            clean_t = _clean_for_reportlab(text)
            story.append(pdf["Paragraph"](clean_t, styles["ultron_body"]))
...
def create_pdf(parameters: dict, player=None) -> str:
    ...
    output_path = _resolve_output_path(parameters.get("output_path"), title, ".pdf", "ultron_ai_output")
    ...
    if action == "create_letter":
        ...
        story.append(pdf["Paragraph"](date_value, styles["ultron_body"]))
        story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        if recipient:
            story.append(pdf["Paragraph"](recipient, styles["ultron_body"]))
            story.append(pdf["Spacer"](1, 0.06 * pdf["inch"]))
        story.append(pdf["Paragraph"](salutation, styles["ultron_body"]))
        story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        for para in _normalize_list(parameters.get("paragraphs")) or [p.strip() for p in re.split(r"\n\s*\n", str(body_text)) if p.strip()]:
            story.append(pdf["Paragraph"](str(para), styles["ultron_body"]))
            story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        story.append(pdf["Spacer"](1, 0.18 * pdf["inch"]))
        story.append(pdf["Paragraph"](closing, styles["ultron_body"]))
        story.append(pdf["Spacer"](1, 0.3 * pdf["inch"]))
        story.append(pdf["Paragraph"](signature, styles["ultron_body"]))
    else:
