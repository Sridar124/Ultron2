...
class GoogleCalendarEngine:
    ...
    def list_events(cls, days: int = 7) -> str:
        """Lists events from Google Calendar if OAuth exists, or Ultron's local calendar."""
...
class GoogleDriveEngine:
    ...
    def search_files(cls, query: str) -> str:
        """Searches Google Drive or local Ultron AI generated files."""
        # Search Desktop/UltronAI folder
        desktop_ai = Path.home() / "Desktop" / "UltronAI"
        ...
        if matches:
            return "Found files in Ultron Workspace:\n" + "\n".join(matches)
    ...
    def upload_file(cls, local_path: str) -> str:
        ...
        # Store in UltronAI cloud staging
        dest = Path.home() / "Desktop" / "UltronAI" / p.name
        try:
            import shutil
            shutil.copy2(p, dest)
            return f"File '{p.name}' uploaded to Ultron Workspace storage."
        except Exception as e:
...
def google_workspace(
    ...
    if service == "gmail":
        if action in {"list", "messages", "inbox", "get_messages"}:
            ...
            subject = params.get("subject", "Message from Ultron AI")
