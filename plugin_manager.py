...
class PluginManager:
    ...
    def register_ultron(self, ultron_obj) -> None:
        self.ultron = ultron_obj
        for p in list(self.plugins):
            try:
                fn = getattr(p, "on_ultron_created", None)
                if callable(fn):
                    fn(ultron_obj)
            except Exception as exc:
                print(f"[Plugins] on_ultron_created error: {exc}")
    ...
    def dispatch(self, hook: str, *args, **kwargs):
        ...
        for p in list(self.plugins):
            try:
                ...
                if callable(fn):
                    # call with ultron if plugin expects it
                    try:
                        res = fn(*args, **kwargs, ultron=self.ultron)
                    except TypeError:
                        res = fn(*args, **kwargs)
                    if res is True:
                        return True
